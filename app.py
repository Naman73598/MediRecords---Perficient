...
def insert():
    ...
    cur = mysql.connection.cursor()
    try:
        sql = "INSERT INTO Records (id,name, age, gender, bloodtype, medicalcondition, doctor, hospital, insuranceprovider, billingamount, roomnumber, admissiontype, medication, testresults, doctorid, doctorspecialization) " \
...
def insert_json_data():
    try:
        ...
        if response.status_code == 200:
            ...
            df_filtered = df[df['Billing Amount'] >= 500]
            data = [(row['ID'], row['Name'], row['Age'], row['Gender'], row['Blood Type'], 
